...
export const Login = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      let result;
      if( checked ){
        result = await axios.post("http://localhost:3001/auth/doc/login", { username, password });
      }
      else{
        result = await axios.post("http://localhost:3001/auth/login", { username, password });
      }
