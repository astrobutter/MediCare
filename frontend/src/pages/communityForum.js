...
export const CommunityForum = () => {
  const userID = useGetUserID();
    const { isDoctor, user, doc, search, setPosts, setTotalPages,} = UserAuth();
    ...
    <motion.div className='page community-forum' initial={{ opacity: 0, x: '100%' }} animate={{ opacity: 1, x: '0%' }} exit={{ opacity: 0, x: '100%' }}>
        <div className="community-nav">
            ...
            <div className="right-field">
                ...
                <button type="button" onClick={() => {search(searchText)}}>Search</button>
            </div>
        </div>
      ...
      <div className="pagination">
        ...
        <button onClick={handlePrevPage} disabled={currentPage === 1}>
          <img src="https://img.icons8.com/small/128/2266EE/left.png" alt="left"/>
        </button>
        <div className="current">{currentPage}</div>
        <button onClick={handleNextPage} disabled={currentPage === totalPages}>
          <img src="https://img.icons8.com/small/128/2266EE/right.png" alt="left"/>
        </button>
